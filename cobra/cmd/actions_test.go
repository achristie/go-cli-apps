...
func TestHostActions(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "AddAction", args: []string{"host1", "host2"}, expectedOut: "Deleted host: host1\nDeleted host: host2\n", initList: true, actionFunction: deleteAction},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tf, cleanup := setup(t, hosts, tt.initList)
			defer cleanup()

			var out bytes.Buffer

			if err := tt.actionFunction(&out, tf, tt.args); err != nil {
				t.Fatalf("Expected no error, got %q\n", err)
			}

			if out.String() != tt.expectedOut {
				t.Errorf("Expected output %q, got %q\n", tt.expectedOut, out.String())
			}
		})
	}
}
